...
public class VoidPearlEntity extends ThrownItemEntity{
    public VoidPearlEntity(EntityType<? extends EnderPearlEntity> entityType, World world) {
    ...
    private void dealDamage() {
        if (!this.getWorld().isClient()) {
            ...
            for (LivingEntity target : targets) {
                ...
                // 💀 damage scales with closeness
                float damage = (float) (6.0f * strength + 2.0f);

                target.damage(world.getDamageSources().magic(), damage);

                // 🕳️ knockback outward
                target.addVelocity(
                        (dx / dist) * 0.6 * strength,
                        0.3 * strength,
                        (dz / dist) * 0.6 * strength
                );
